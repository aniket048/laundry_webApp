	...
	return (
		<div className="p-5 single-order">
					...
					{props.viewProduct.map((item) => (
						<tr key={item.id}>
							...
							<td>
								...
								<button
									className="mt-0 p-2 ml-2 btn btn-info text-white"
			...
			<Modal toggle={() => setDeleteModal(false)} isOpen={deleteModal}>
				<h4 className="modal-title text-info ml-4">Are you want to delete this product?</h4>

				<div className="modal-footer mt-3">
					<Button color="warning" type="button" onClick={() => setDeleteModal(false)}>
						Close
					</Button>
					<Button color="info" type="button" onClick={handleDeleteProduct}>
						Confirm
					</Button>
				</div>
			</Modal>
